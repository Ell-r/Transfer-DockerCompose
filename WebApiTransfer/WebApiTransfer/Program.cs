...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowTwoDomains", policy =>
    {
        policy.WithOrigins("http://localhost:5173",
            "http://againmysuperultrasite.somee.com")
              .AllowAnyHeader()
              .AllowAnyMethod()
              .AllowCredentials();
    });
...
using (var scoped = app.Services.CreateScope())
{
    ...
    if (myAppDbContext.Users.Any(u => u.UserRoles.Any(r => r.RoleId == 2)))
    {
       ...
       .GetRequiredService<IEmailSenderService>();
        ...
        {
            await emailSenderService.SendEmailAsync(admin.Email, "Server", $"Сервер {builder.Configuration.GetSection("Server").GetValue<string>("SiteUrl")} успішно запущений");
        }
    }
}
